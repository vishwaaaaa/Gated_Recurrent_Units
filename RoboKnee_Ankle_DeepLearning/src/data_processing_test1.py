...
def get_lookback_windows(data_structure, sequence_length, constants):
    ...
    for n, file in enumerate(data_structure):
        ...
        # Retrieve Output
        #ankle_torque = data_structure[file]['ankle_angle_l_moment']
        ankle_torque = data_structure[file][['ankle_angle_l_moment','knee_angle_l_moment']]
        # # Retrieve States
        # omega_motor = data_structure[file]['Motor Velocity (rpm)']
        # hip_position = data_structure[file]['Hip Angle (rad)']
        # ankle_position = data_structure[file]['Ankle Angle (rad)']
        ...
        # Retrieve Input
        # input_data = data_structure[file].drop(columns=['ankle_angle_l_moment','knee_angle_l_moment','time'])  # Remove ankle torque column
        # U = input_data  # Assign the remaining columns to U
    #     U = data_structure[file]['hip_rotation_r', 'knee_angle_r', 'ankle_angle_r','hip_rotation_velocity_r', 'knee_velocity_r', 'ankle_velocity_r',
    #    'hip_rotation_velocity_l']
        U = data_structure[file][['hip_rotation_r', 'knee_angle_r', 'ankle_angle_r','hip_rotation_velocity_r', 'knee_velocity_r', 'ankle_velocity_r', 'hip_rotation_velocity_l','RTA','RRF','RBF','RGMED','RMGAS','RVL','RGRAC','RGMAX']]#,'RShank_ACCX','RShank_ACCY','RShank_ACCZ','RShank_GYROX','RShank_GYROY','RShank_GYROZ','RPelvis_ACCX','RPelvis_ACCY','RPelvis_ACCZ','RPelvis_GYROX','RPelvis_GYROY','RPelvis_GYROZ']]
        ...
        # print(np.array(input_data.columns).shape)
        # print(np.squeeze(tmp_x, axis=0).shape)
        # data_x = pd.DataFrame(np.transpose(tmp_x.reshape(tmp_x.shape[0], -1)),columns=input_data.keys())
        data_x = pd.DataFrame(np.squeeze(tmp_x, axis=0), columns=input_data.columns)

        tmp_y = np.array([ankle_torque])
        # print(np.transpose(tmp_y).shape)
        data_y = pd.DataFrame(np.squeeze(tmp_y),columns=['Taua','Tauk'])
        ...
        for i, signal_name in enumerate(list(data_y.columns)):
            # for debugging
            temp= data_y[signal_name].shift(-prediction_horizon).fillna(method="ffill").values
